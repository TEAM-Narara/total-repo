...
interface ListAPI {
    ...
    @POST("api/v1/list")
    suspend fun createList(@Body listRequestDto: ListRequestDto): Response<ApiResponse<ListResponseDto>>

    @PATCH("api/v1/list/{listId}")
    suspend fun updateList(
        @Path("listId") listId: Long,
        @Body listRequestDto: ListRequestDto
    ): Response<ApiResponse<ListResponseDto>>

    @PATCH("api/v1/list/{listId}/archive")
    suspend fun setListArchive(@Path("listId") listId: Long): Response<ApiResponse<ListResponseDto>>
