...
class BoardValidatorTest {
    ...
    @ParameterizedTest
    @CsvSource({
            ", '{\"type\": \"color\", \"value\": \"#ffffff\"}'",  // 이름이 없는 경우
            "'', '{\"type\": \"imageurl\", \"value\": \"https://example.com/image.jpg\"}'"  // 이름이 빈 문자열인 경우
    })
    void testBoardEntityCreation(String name, String backgroundJson) {
        // JSON을 Map으로 변환
        ObjectMapper objectMapper = new ObjectMapper();
        Map<String, Object> background = objectMapper.readValue(backgroundJson, new TypeReference<Map<String, Object>>() {});

        BoardCreateRequestDto boardCreateDto = new BoardCreateRequestDto(name, background);
